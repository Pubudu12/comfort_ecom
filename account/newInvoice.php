...
if( ($checkuser['access'] == 1 ) | ($checkuser['access'] == 2 )){
    ...
    $selectQuery = "SELECT o.*,dd.*, refShM.`name` shippingMethod
                    ...
                    WHERE o.`order_no`='$order_no' AND o.`user_type`='std' ";
    }else{
        // if guest users
        ...
        $selectQuery = "SELECT o.*,dd.*, refShM.`name` shippingMethod
                            FROM `orders` o 
                            INNER JOIN `delivery_details` AS dd ON dd.`order_no` = o.`order_no` AND dd.`email` = '$email'
                            LEFT JOIN `ref_shipping_methods` AS refShM ON refShM.`id` = dd.`shipping_method`
                            WHERE o.`order_no`='$order_no' AND o.`user_type`='guest' ";
    ...
    $query = "SELECT oi.*,p.`name`, p.`id` product_id, thumb.`name` AS thumb
            ...
            LEFT JOIN `product_images` AS thumb ON thumb.`product_id` = p.`id` AND thumb.`type` = 'thumb'
...
<body class="">
    ...
    <form action="" method="GET" id="redirect_payment_gateway">
                    ...
                    <div class="col-12 order_customer_details_box pb-30">
                        <div class="row">
                            <div class="col-md-6 col-sm-12">
                                <h1 class="text-left"><?php echo $order_no ?></h1>
                            </div>
                            <div class="col-md-6 col-sm-12">
                                <h1 class="text-right">22 April 2020</h1>
                            </div>
                        </div>
                        <hr>
                        <div class="row">
                            <div class="col-md-6 text-left">
                                ...
                                <h4>
                                    Ann <br>
                                    No 43,Main Road,Colombo-32144 <br>
                                    0114322342 <br>
                                    ann@gmail.com <br>
                                </h4>
                            </div>
                            <div class="col-md-6 text-right">
                                ...
                                <?php foreach ($orderDeliveryDetails as $key => $value) { ?>
                                    <tr>
                                        <td><?php echo $key?></td>
                                        <td>:</td>

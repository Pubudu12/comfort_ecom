...
while( $row = mysqli_fetch_array($query) ) {  // preparing an array
        ...
        $edit_btn = '<a href="'.URL.'promo/update?id='.$id.'" class="btn btn-primary fa fa-edit" title="Edit Code"></a>';
        $delete_btn = ' <button  class="btn btn-sm btn-danger fa fa-trash-o"
                        onclick="deleteItem(this)"
                        data-after-success=3
                        data-id="'.$id.'"
                        data-refresh=dataTableRefresh
                        data-url="'.URL.'promoCodes/ajax/controller/deletePromoController.php"
                        data-key="delete_promo_code"></button>';

...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(isset($_POST['create_gl_category'])){
        ...
        if($validated_data === false) {
            ...
            $DBResult = $dbOpertionsObj->insert('gallery_category', $insertData);

            $error = $DBResult;
            if($DBResult['result']){

                $result = 1;
                $message = 'New Gallery Category '.$validated_data['name'].' has been created';

                $redirectURL = URL.'gallery/categories';

                $auditArr = array(
                    'action' => 'Create Gallery Category',
                    'description' => $message
                );
